...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:sml="http://www.opengis.net/sensorML/1.0.1"
	...
	<pattern id="SystemValidation">
		...
		<rule context="//sml:System">
			<assert
				test="count(sml:capabilities/swe:DataRecord/swe:field/swe:Envelope[@definition = 'urn:ogc:def:property:OGC:1.0:observedBBOX']) = 1">Error: one "swe:field" of a "swe:DataRecord" in a
				"sml:capabilities" has to contain a
	...
	<pattern id="GeneralValidation">
		...
		<rule context="//sml:classification">
			<assert
				...
				Error: one classifier for the sensorType classification scheme has to be
				given!
			</assert>
			<assert
