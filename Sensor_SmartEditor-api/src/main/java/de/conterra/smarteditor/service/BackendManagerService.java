...
public class BackendManagerService {
    ...
    /**
     * Initializes the backend beans based on the resource type
     * Standard templates will be used for initializing the content
     *
     * @param pResourceType one of MD_ScopeCode
    ...
    public synchronized Document mergeBackend() {
        if (backend != null && backend.getStorage() != null && getMergeDocument() != null) {
    ...
    public String getResourceType() {
        if (getMergeDocument() != null) {
           ...
           String resourceType= lUtil.evaluateAsString("//gmd:hierarchyLevel/*/@codeListValue", getMergeDocument());
            if(resourceType==""){
            XPathFactory factory=	XPathFactory.newInstance();
            XPath  xpath=factory.newXPath();
            	try {
					XPathExpression expr=xpath.compile("//sml:SensorML/sml:member/sml:identification/sml:IdentifierList/sml:identifier[name='longName']");
					resourceType=expr.evaluate(getMergeDocument());
